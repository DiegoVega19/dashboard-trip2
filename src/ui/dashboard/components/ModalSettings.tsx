...
const Modal: React.FC<ModalProps> = ({ closeModal, onSubmit, defaultValue }) => {
  const fields: string[] = Object.keys(Object.values(dataJSON)[0]).filter(
  ...
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    if (
      ...
      parseInt(formState.criterion) > 1 &&
      parseInt(formState.criterion) < 4
    ) {
          ...
          <form onSubmit={handleSubmit}>
            ...
            {errors.filter((item: string) => item.startsWith("INVALID_ID")).length > 0 && (
              <>
                <br />
                <div className="error">
                  {errors.filter((item: string) => item.startsWith("INVALID_ID"))[0].replace("INVALID_ID_", "")} is
                  not a valid bond
                </div>
              </>
            )}
            {errors.filter((item: string) => !item.startsWith("INVALID_ID")).length > 0 && (
              <div className="error">Please input {errors.filter((item: string) => !item.startsWith("INVALID_ID")).join(", ")}</div>
            )}
