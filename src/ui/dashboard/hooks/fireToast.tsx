...
if (alertSettings){
  for (const alertSetting of JSON.parse(alertSettings)) {
    ...
    if (alertSetting.id=="ALL"){
      ...
      {
        {
          const msg=`${alertSetting.para} of ${id} ${alertSetting.criterion==0?"goes down by":alertSetting.criterion==1?"goes up by":alertSetting.criterion==2?"is smaller than":alertSetting.criterion==3?"is greater than":"is equal to"} ${1}`;
